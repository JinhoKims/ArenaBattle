...
AABSection::AABSection()
{
	...
	Trigger->OnComponentBeginOverlap.AddDynamic(this, &AABSection::OnTriggerBeginOverlap); // 컴포넌트의 델리게이트에 OnTriggerBeginOverlap() 함수를 바인딩 해준다.
	...
	for (FName GateSocket : GateSockets) // 동서남북 4번 반복
	{
		...
		NewGateTrigger->OnComponentBeginOverlap.AddDynamic(this, &AABSection::OnGateTriggerBeginOverlap); // 컴포넌트의 델리게이트에 OnTriggerBeginOverlap() 함수를 바인딩 해준다.
		NewGateTrigger->ComponentTags.Add(GateSocket); // 어떤 문인지 구분할 수 있도록 태그를 추가한다.(x+ ~ y-)
	}
...
{
	GetWorld()->SpawnActor<AABCharacter>(GetActorLocation() + FVector::UpVector * 88.0f, FRotator::ZeroRotator); // 스폰 액터(NPC)
}
