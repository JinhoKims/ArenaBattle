...
/*	UE_LOG(ArenaBattle, Warning, TEXT("Actor Name : %s, ID : %d, Location X : %.3f"), *GetName(), ID, GetActorLocation().X); // (로그명, 로그 중요도, 사용자텍스트)
	ABLOG_Long(Warning, TEXT("Actor Name : %s, ID : %d, Location X : %.3f"), *GetName(), ID, GetActorLocation().X); // 로그 타입 2 호출 */
	ABLOG_Short(Warning); // 로그 타입 1 호출
}
